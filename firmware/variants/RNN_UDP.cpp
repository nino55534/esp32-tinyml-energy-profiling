...
static void udp_send(const char* msg) {
    ...
    if (sock < 0) { ESP_LOGE(TAG, "socket greska"); return; }
...
void register_ops() {
    ...
    ESP_LOGI(TAG, "RNN ops registrirani");
}
...
float pokreni_rnn() {
    ...
    if (input->type == kTfLiteInt8) {
        ...
        ESP_LOGI(TAG, "RNN Input scale: %f, zp: %d", scale, zero_point);
        for (int i = 0; i < 24; i++) {
...
static void ciklus() {
    ...
    gpio_set_level(TRIGGER_PIN, 1);
    int64_t t_udp_start = esp_timer_get_time();

    char payload[80];
    snprintf(payload, sizeof(payload), "{\"temp\":%.2f,\"rnn\":%.2f}", temp, rnn_out);
    udp_send(payload);

    int64_t t_udp_end = esp_timer_get_time();
    gpio_set_level(TRIGGER_PIN, 0);
    ESP_LOGI(TAG, "[UDP] %lld us", (long long)(t_udp_end - t_udp_start));
...
extern "C" void app_main() {
    ...
    ESP_LOGI(TAG, "Pokrenuto mjerenje ESP32-S3 (samo RNN)");
